...
import { TASK_BROWSE_FILTER_CATEGORIES } from '@/app/(task)/context'
...
export function TaskBrowseDataProvider({
  children,
  headerSubtitle,
  initialTasks,
}: {
  ...
  const value = useMemo<TaskBrowseDataContextValue>(
    () => ({
      sort,
      setSort,
      cycleSort,
  ...
  return (
    <TaskBrowseDataContext.Provider value={value}>
      {children}
...
export function useTaskBrowseData() {
  ...
  if (!ctx) {
    throw new Error(
      'useTaskBrowseData must be used within TaskBrowseDataProvider',
    )
  }
  return ctx
}
