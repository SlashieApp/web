...
function getTaskStage(task: TaskItem): TaskStage {
  ...
  if (isOfferAwarded(task.status)) return 'awarded'
  if (task.offers.length === 0) return 'draft'
  return 'open'
}
...
function TaskCard({ task }: { task: TaskItem }) {
  ...
  const amount =
    getOfferRange(task.offers) ?? formatPounds(task.priceOfferPence ?? 0)
...
export default function CustomerRequestsPage() {
  ...
  const quoteActivityCount = useMemo(
    () => activeTasks.filter((t) => t.offers.length > 0).length,
    [activeTasks],
  )
  const totalQuotes = useMemo(
    () => myPostedTasks.reduce((n, t) => n + t.offers.length, 0),
    [myPostedTasks],
  )
