...
- **`.env` is gitignored.** The update script recreates it from injected environment secrets (`NEXT_PUBLIC_GRAPHQL_URL`, `SCHEMA_ACCESS_TOKEN`, `NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN`, `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN`, `NEXT_PUBLIC_POSTHOG_HOST`). See `.env.example` for the full list.
