...
export function PostedTaskCalendar() {
  const {
    taskRows,
  ...
  const marks = useMemo(
    () => calendarMarksForMonth(taskRows, year, month),
    [taskRows, year, month],
  )
      ...
      <Stack gap={4}>
        ...
        <Grid templateColumns="repeat(7, 1fr)" gap={1}>
          ...
          {cells.map((cell) => {
            ...
            const dayMarks =
              marks.get(cell.day) ?? new Set<PostedTaskCalendarMark>()
...
function CalendarDot({ tone }: { tone: PostedTaskCalendarMark }) {
  return (
    <Box
      w="6px"
      h="6px"
